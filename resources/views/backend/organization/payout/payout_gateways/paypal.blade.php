@php
    $data = organizationPayoutMethod(authOrganizationId(), 'paypal');
@endphp
...
<div class="col-xxl-6 col-md-12">
    <div class="payment-box payment-box-v2">
        ...
        <div class="payment-settings ms-sm-auto">
            <div class="payment-settings-btn">
                <a href="#" class="btn btn-md sg-btn-outline-primary" data-bs-toggle="modal"
                    data-bs-target="#paypalConfiguration"><i class="las la-cog"></i> <span>Setting</span></a>
            </div>
        </div>
        ...
        <div class="payment-checker">
                ...
                <div class="setting-check">
                    @if (!blank($data))
                        <input type="checkbox" class="instructor-status-change"
                            {{ $data->is_default == 1 ? 'checked' : '' }} data-id="{{ $data->id }}"
                            value="payout-default-change/{{ $data->id }}" id="customSwitch1-{{ $data->id }}">
                        <label for="customSwitch1-{{ $data->id }}"></label>
                    @else
                        <input type="checkbox" class="instructor-status-change" data-id=""
                            value="payout-default-change/" id="paypal1">
            ...
            <div div class="d-flex justify-content-between gap-40 mt-20">
                ...
                <div class="setting-check">
                    @if (!blank($data))
                        <input type="checkbox" class="instructor-status-change"
                            {{ $data->status == 1 ? 'checked' : '' }} data-id="{{ $data->id }}"
                            value="payout-status-change/{{ $data->id }}" id="customSwitch2-{{ $data->id }}">
                        <label for="customSwitch2-{{ $data->id }}"></label>
                    @else
                        <input type="checkbox" class="instructor-status-change" data-id=""
                            value="payout-status-change/" id="paypal2">
...
<div class="modal fade" id="paypalConfiguration" tabindex="-1" aria-labelledby="paymentMethodLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form action="{{ route('organization.payouts.method-setting-update') }}" method="post" class="form">
                @csrf
                <div class="row gx-20">
                    <input type="hidden" name="is_modal" class="is_modal" value="0">
                    <input type="hidden" name="payout_method" value="paypal">
                    <input type="hidden" name="organization_id" value="{{ authOrganizationId() }}">
                    <div class="col-sm-12">
                        <div class="mb-4">
                            <label class="form-label">{{ __('paypal_email') }}</label>
                            <input type="text" class="form-control rounded-2" name="value[email]"
                                placeholder="{{ __('kenzi.lawson@example.com') }}"
                                value="{{ $data ? $data->value['email'] : '' }}">
