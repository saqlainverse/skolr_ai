...
class InstructorDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function ($user) {
                return view('backend.admin.instructor.status', compact('user'));
            })->addColumn('action', function ($user) {
                return view('backend.admin.instructor.action', compact('user'));
            })->addColumn('name', function ($user) {
                return view('backend.admin.instructor.name', compact('user'));
    ...
    public function query(): QueryBuilder
    {
        ...
        return $model->with('instructor', 'lastActivity')->whereHas('instructor')->when($id, function ($query) use ($id) {
            $query->whereHas('instructor', function ($query) use ($id) {
